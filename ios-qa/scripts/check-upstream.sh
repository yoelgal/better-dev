...
#
# Reimplemented for better-dev (pattern credit: gstack's sync check).
set -u
...
while read -r _kw pinned path; do
  # local daemon/src/X mirrors upstream ios-qa/daemon/src/X
  url="$raw_base/HEAD/ios-qa/$path"
  out="$tmpdir/fetched"
  ...
  if [ "$current" = "$pinned" ]; then
    ...
    echo "DRIFT: upstream changed security file ios-qa/$path (pinned at commit $commit)"
    drift=1
  fi
done < "$tmpdir/pins"
