...
- `.better-dev/` holds this repo's better-dev data: `overrides.md`, `rules.md`, and one
  ...
  state.
- Update the tool with `/update` - it pulls the global clone and reconciles skill links when a release
  added or removed one. Re-run `/onboard` any time to wire in what's missing.
...
`scripts/`, `docs/`, `friction/`, and the gitignored `raw/` research archive are its own parts, not
separate tools. `skills/` is the product; `scripts/` holds the four helpers that survive because no
host ships them - the package gate, the uninstaller, the skill staging tool, and the review packager.
There is one version, stamped in `.claude-plugin/plugin.json`, and one release ledger,
`docs/RELEASES.md`. Branch discipline is trunk: `feat/*` off `main`, merged back to `main`, released by
a version commit and a tag. `main` is the distribution channel - a merge is live to every user on
their next `git pull` - so a red gate reaches people, and that is the cost trunk trades the staging
buffer for.
