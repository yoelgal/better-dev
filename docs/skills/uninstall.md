...
Removes what better-dev installed - one repo's managed discovery block in `CLAUDE.md` / `AGENTS.md`,
or the global per-host skill install - and leaves everything else untouched. It never deletes your
rules, overrides, or loop history unless you explicitly ask for that too; the default unwire keeps
your project's accumulated data in place.
...
the fallback the routing block itself carries for this case: run the clone's `scripts/bd-uninstall repo`
...
- A dry-run (the default, no `--yes`) prints exactly what would be removed and changes
  nothing on disk.
- After a repo unwire, your recorded rules, overrides, and loop history are still
