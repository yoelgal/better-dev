...
0.2.0 reonboard - **better-dev no longer writes into your repos, and a repo wired under 0.1.x should be cleaned up.** The discovery block is deleted: it was written to files your agent may never read, and measurement showed it barely mattered anyway - 28 of 33 skills already read the project's recorded decisions themselves, and this library ran a full session with its own block unread while nothing degraded. Recorded decisions now live in your harness's durable memory instead of a file. Run `/onboard` once per wired repo to remove the block and the comms pointer it left behind; `.better-dev/` stays for the loop's ledger and nothing else. Two things to know: durable memory is per-machine, so a decision no longer travels to a colleague, and on omp it needs `memory.backend` set before a write lands at all.
