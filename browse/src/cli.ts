...
Refs:           After 'snapshot', use @e1, @e2... as selectors:
  ...
  if (command === 'connect') {
    ...
    try {
      // Start server in headed mode on a well-known port
      ...
      // ponytail: sidebar-agent + terminal-agent (PTY) spawns removed in the
      // better-dev vendored cut (sidebar/PTY glue; see browse/UPSTREAM).
    } catch (err: any) {
