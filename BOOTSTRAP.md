...
First check whether it's already installed: look for a better-dev entry in the host's global skills
directory (e.g. `~/.claude/skills/better-dev`, `~/.codex/skills/better-dev`) or an existing clone. If
it's there, run `git pull` in the clone to update and move to step 3 — nothing is installed twice.
...
- **Any host, including Claude** — clone once to a stable location and run its installer:
  ...
  The installer links the tool into the host's global skills directory (`~/.claude/skills/better-dev`,
  `~/.codex/skills/better-dev`, and so on), copying instead of linking where symlinks aren't available.
