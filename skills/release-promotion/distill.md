...
Read both stores with the `read` tool: `memory://root/learned.md` for the lessons the sessions
recorded, `.better-dev/rules.md` for the rules already promoted.
...
- **ADD** - a lesson that recurred across two or more work-items, or that a verified fix confirmed,
  has earned a rule: add the line to `.better-dev/rules.md` with `edit`. The negative-lesson filter
...
any of them - propose, never auto-edit someone's memory. And leave the lesson store alone:
consolidation happens on the rules side, by promoting into `.better-dev/rules.md` and retiring stale
rules there, never by rewriting what the sessions recorded. Nothing recurred or went stale? That's a
