...
- Walk step one against the state that summons the skill. Name what a finished run leaves fixed, removed,
  ...
  same thing in both lists means the mechanism's reach stops at exactly the machines that need it: a
  repair step that locates its own target by guessing at the host's layout - globbing a home directory for
  the conventional folder, assuming the conventional filename - misses the machine whose layout is
  unconventional, and that is the machine most likely to be the broken one, so the fix the step was
  carrying never arrives at the only place that needed it. The shape of that failure is worse than the
  failure: nothing errors, the step simply finds no target, and the silence reads exactly like a machine
  with nothing to repair. Grade it by putting the target in its worst *recorded* failure state -
  `docs/TRAPS.md`, a ruling in `docs/DECISIONS.md`, a prior PR's own findings, never an imagined one - and
  running step one against it for real. The remedy is rarely self-sufficiency, which usually cannot be
  built: name at that step the route whose inputs come from outside the target - a structural handle that
  already carries the answer (a sentinel a skill wrote names its own target: `/onboard`'s `BEGIN`/`END`
  markers locate its managed block in whatever entry file the repo turned out to use, so a later pass
  edits or removes that block without guessing at a layout), an operator-supplied value, or a front door
  that assumes nothing is installed.
...
A structural check proves a skill is well-formed. It does not prove the skill works. Three cheap checks
close the gap - no harness, no fixtures, one manual run each - and the last two are what
`/self-extension` runs before it promotes:

- **Well-formed** - valid frontmatter, a kebab name matching the folder, a description that states its
  trigger, no `@`-links or stray keys. That list is the bar wherever a skill is authored, including in the
  operator's own repo; in a checkout of this repo, `scripts/bd-skill-stage lint` is how a maintainer
  checks it.
