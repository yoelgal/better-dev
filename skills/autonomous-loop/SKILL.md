...
5. **Record.** Append a receipt - tried / result / learned / plan-delta - to `receipts.md`, the *result*
   ...
   `bd-mem learn` (a papercut is an annoyance the operator triages, not a lesson recall replays). A `receipts.md` still at pass 0 after several implementation passes is the tell that recording
